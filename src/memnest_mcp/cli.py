...
_PERSIST_PROMPT = (
    ...
    "routine facts 2-3.\n"
    "5. LINK THE MEMORIES — this is the highest-value step and the easiest to "
    "skip. Without edges you are using a graph database as a flat list, and "
    "similarity search alone cannot tell that one fact replaces another. "
    "After storing, create an edge whenever:\n"
    "   - the new memory corrects/updates something already stored -> store it "
    "with memory_store {\"content\": \"...\", \"supersedes\": <old_id>} in ONE "
    "call. Do not store then relate: a correction is near-identical to what it "
    "corrects, so a plain store can be silently absorbed by semantic dedup and "
    "the old version lost. Passing supersedes disables dedup for that store and "
    "creates the edge, so search ranks the new version above the old one\n"
    ...
    "memory_keep_separate(memory_ids=[a, b]), or "
    "memory_keep_separate(pairs=[[a, b], [c, d]]) for several independent "
    "pairs at once. It records that verdict without "
...
_DREAM_PROMPT = (
    ...
    "{\"memory_ids\": [<a>, <b>]} for ONE cluster, or with "
    "{\"pairs\": [[<a>, <b>], [<c>, <d>]]} to resolve several clusters in one "
    "call. Use `pairs` for more than one cluster — `memory_ids` is a clique, so "
    "passing every cluster's ids together would record verdicts on "
    "cross-cluster pairs you never examined and permanently hide real conflicts "
    "between them. That records the verdict WITHOUT creating an "
    ...
    "To read the full text of several memories a cluster or contradiction "
    "names, call memory_get with {\"memory_id\": [<a>, <b>, <c>]} — it accepts "
    "a list, so this is one call rather than one per id.\n\n"
